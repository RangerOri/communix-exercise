...
namespace GameLogic
{
	...
	public class Paddle : MonoBehaviour
	{
		...
		void Movement()
		{
			...
			{                                                           //Is the paddle able to move?
				...
				{                               //Is the left arrow key currently being pressed
					rig.velocity = new Vector2(-1 * speed * Time.fixedDeltaTime, 0);     //Set the paddle's rigidbody velocity to move left
				}
				else if (Input.GetKey(KeyCode.RightArrow))
				{                           //Is the right arrow key currently being pressed
					rig.velocity = new Vector2(1 * speed * Time.fixedDeltaTime, 0);      //Set the paddle's rigidbody velocity to move right
				}
				else
				{
		...
		{
			if (col.gameObject.tag == TAG_BALL)
